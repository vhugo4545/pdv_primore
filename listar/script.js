    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 401 || response.status === 404) {
                alert('Erro de recurso não encontrado. Redirecionando para a página de login.');
        ...
        if (data.pedidos && Array.isArray(data.pedidos)) {
            ...
            throw new TypeError('Resposta da API não contém a chave "pedidos" ou não é um array.');
        }
    } catch (error) {
